...
class LocationManager: NSObject {
    ...
    var curLoc: CLLocation? = ArchiveService.instance.getLastStoredLocation() {
        didSet {
            NotificationCenter.default.post(name:
                NSNotification.Name(rawValue: locationUpdateNotification), object: nil)
        }
    }
    ...
    private func displayAlert(_ controller: UIAlertController) {
        guard let viewController = UIApplication.shared.keyWindow?.rootViewController else {
            // TODO: handle fatal error if ! location services available
            return
        }
